---
title: "experiment code 0517 with IBM"
...
for(i in 1:2)
{
  ...
  model<-svm(Churn~.,data=trainset,kernel="radial",cost=1,
            gamma=1/ncol(trainset),probability=TRUE)
  pred<-predict(model,trainset,decision.values =TRUE,probability =TRUE)

  chst<-trainset$Churn
  churnP<-attr(pred,"probabilities")[,2] 
  ...
  #测试集
  #预测
  churn.predict <-predict(model,testset,decision.values =TRUE,probability =TRUE)
  chrate<-attr(churn.predict,"probabilities")[,2]  
